...
import "../../src/libraries/LinearBondingCurve.sol";
...
contract LaunchpadTest is Test {
    ...
    function testPurchaseTokens() public {
        // Create fundraiser
        uint256 targetFunding = 1000_000 * 10 ** 6; // 100K USDC
        launchpad.createFundraise(targetFunding, block.timestamp + 1, "Test", "TEST");
        ...
        usdc.approve(address(launchpad), 10 ** 6);
        launchpad.purchaseTokens(1, 10 ** 6);
        console2.log("balance", launchpad.userPurchases(1, user2));
        vm.stopPrank();

        vm.startPrank(user3);
        usdc.approve(address(launchpad), 10 ** 6);
        launchpad.purchaseTokens(1, 10 ** 6);
        console2.log("balance", launchpad.userPurchases(1, user3));
        vm.stopPrank();
